...
for gamma in gamma_list:
    for Lambda in lambda_list:
        ...
        test_acc = accuracy_score(y_test_labels, y_test_pred_labels) * 100
        ...
        print(f'Test Accuracy: {test_acc:.2f} | Gamma: {gamma:.7f} | Lambda: {Lambda:.7f}')

data = pd.DataFrame(results, columns = ['Gamma', 'Lambda', 'Accuracy'])
data.to_csv('SignFi-Lab_SenHyp.csv', index = False)
